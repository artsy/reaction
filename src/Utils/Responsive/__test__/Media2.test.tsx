...
import { createMedia } from "../Media2"

const {
  MediaComponent: Media,
  MediaContextProvider: ContextProvider,
} = createMedia({
  breakpoints: {
    "extra-small": 0,
    small: 768,
  ...
  describe("with a context", () => {
    it("renders only matching `at` breakpoints", () => {
      const query = renderer.create(
        <ContextProvider onlyRender={["extra-small", "small"]}>
    ...
    it("renders only matching `lessThan` breakpoints", () => {
      const query = renderer.create(
        <ContextProvider onlyRender={["small", "medium"]}>
    ...
    it("renders only matching `between` breakpoints", () => {
      const query = renderer.create(
        <ContextProvider onlyRender={["medium", "large"]}>
